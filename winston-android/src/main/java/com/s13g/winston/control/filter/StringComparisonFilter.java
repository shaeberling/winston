...
/**
 * Defines a string comparison filter with the possibility for multiple filters
 * (e.g. EditDistance, SubstringMatching, LCS).
 */
public class StringComparisonFilter implements IOperationFilter {
     ...
     * Constructor
     *
     * @param filter        the filter to use for the string comparison.
     * @param knownOperations an array of known words. The filter method will use this word
     * @param tolerance     this value represents the allowed variance (equal or less) of the string comparison with known strings.
     *                      For simplicity, currently only one tolerance value to all known data is
    ...
    /**
     * Filters the list of all input value within all known command operations using the specified {@IComparePlugin}
     * and the <code>tolerance</code>.
     * In case the perfect matching command operation (=0 difference) was found , this method stops the lookup
     *
     ...
     * returns the number of required changes that have to be  done to match both given strings.
     * To match both given strings three actions may be possible:<br>
     * - REPLACE (replace a character with an other)<br>
     * - INSERT (insert a new character)<br>
     * - DELETE (delete an existing character)<br>
     * <br>
     * The string comparison function returns the cost of the optimal alignment, but not the
     * alignment itself. <br> An empty string counts as <code>null</code> string.
     * <br>
     * If for example "you should not" and "thou shalt not" have to be compared the result would be 5.
     *
     ...
     * returns the number of changes that have to be  done to match both given strings. To match
     * both given strings three actions may be possible:<br>
     * - REPLACE (replace a character with an other)<br>
     * - INSERT (insert a new character)<br>
     * - DELETE (delete an existing character)<br>
     * <br>
     * The string comparison function returns the cost of the optimal alignment,
     * but not the alignment itself. <br>
     * <br>
     * If for example "you should not" and "thou shalt not" have to be compared the result would be 5.
     *
     * @param string1 string 1 for the comparison
    ...
    /**
     * Initializes the two dimensional cache array for the dynamic programming
     * based on the given <code>maxLen</code> value
     *
     * @param maxLen the maximum length of the cache
